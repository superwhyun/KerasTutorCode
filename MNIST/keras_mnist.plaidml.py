...
class MNIST_KERAS:
    ...
    def prepare_data(self):
        ...
        self.y_train = keras.utils.to_categorical(y=self.y_train, num_classes=self.number_of_class)
        self.y_test = keras.utils.to_categorical(y=self.y_test, num_classes=self.number_of_class)
    ...
    def create_model(self):
        ...
        self.model.add(keras.layers.Dropout(rate=0.3))

        self.model.add(keras.layers.Flatten())                                            # fully connected layer로 가려면, 2차원 data를 1차원으로 바꿔줘야 하며, flatten layer가 그 역할을 함
        self.model.add(keras.layers.Dense(units=1024, activation='relu'))                 # fully connected layer로 만들어 줌
        self.model.add(keras.layers.Dropout(rate=0.3))
        self.model.add(keras.layers.Dense(units=self.number_of_class, activation='softmax'))
        self.model.compile(loss=self.loss, optimizer=self.optimizer, metrics=self.metrics)
